...
if tema == 'TRANSPORTE':
    ...
    fig.update_yaxes(title=None,tickfont_color='black')
    fig.update_xaxes(title=None,tickfont_color='black',tickfont_size=15)
    fig.update_layout(legend=dict(title=None, orientation='h',x=0.3),showlegend=True)
    fig.update_traces(textfont_size=15,textfont_color='black',textposition='top left')
    fig 
    ...
    contvia = transporte[['MÊS','TICKET','CONTROLE']]
    via = contvia.groupby(['MÊS','CONTROLE']).count().reset_index().sort_values('CONTROLE')
    fig = px.bar(via,x='MÊS',y='TICKET',title='HISTÓRICO DE VIAGENS',text='TICKET',width=900,height=400)
    fig.update_yaxes(title=None,tickfont_color='black')
    fig.update_xaxes(title=None,tickfont_color='black')
